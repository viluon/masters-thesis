...
productions of a \acrfull*{peg}. It relies heavily on memoization to avoid
costly backtracking, at the expense of memory overhead.
...
\acrshort*{cfg}s is ambiguous, whereas \acrshort*{peg}s use ordered choice,
which greedily attempts to match alternatives in order. The right-hand side of a
...
steps, which is not surprising, considering the relative complexity of the
languages that the individual steps recognize and process.

Our design features automatic error recovery based on the grammar definition.
Rules can fail to match for several reasons:
\begin{itemize}
	\item An input token is missing where it is expected. For example, a missing
	semicolon after a declaration. The parser could recover by inserting the
	expected token at the current location.
	\item An input token is present where it is not expected. For example, a
	semicolon in the middle of an expression. The parser could recover by
	ignoring the unexpected token.
	\item None of the alternatives in a choice match.
\end{itemize}


\subsubsection*{Tracking source locations}

Source locations are an important consideration for any incremental parser. A
naive implementation that includes the source locations in the parse tree leads
to a linear amount of work per edit, as the parser must recompute the source
location of every node. This would largely defeat the purpose of incremental
parsing.
