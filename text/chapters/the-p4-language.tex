...
architecture, as well as certain built-in language constructs\todo{reference the
parser declaration grammar section}. For example, the \extern object in
...
state\todo{context?} in the form of variables and \extern instantiations. These
...
contains an intrinsic\todo{established term?} \extern\todo{we should definitely
change this and the plural into a command} definition called \texttt{packet\_in}
...
instantiated implicitly for every parser parameter\todo{arg vs param} of type
...
is ultimately motivated by the need to control the side effects of \extern
functions and methods, which are arbitrarily powerful. The copy in / copy out
calling convention lets compilers reason about programs in the presence of
\extern{}s.
...
specification lays out\todo{finish}
