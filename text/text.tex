...
to the language server, which updates its model. When VS Code asks for
completions, hover, diagnostics and other features, the analyzer recomputes
necessary information on-demand and responds with the appropriate \acrshort{lsp}
response.
...
that is tolerant to invalid input to the fullest extent possible.
...
starting the parser, however. Consider the code in Listing~\ref{lst:p4pp}. In
this context,
