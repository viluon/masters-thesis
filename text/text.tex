...
Haskell evaluation code lenses.}. Go to Definition and Find References are
...
Markdown and LaTeX\todo{improve this} documents, as well as in programming
language comments. Even though \acrshort{lsp} has no built-in support for
extracting the comments of a document or for spell checking in general,
LT$_\text{E}$X achieves this with a combination of non-\acrshort{lsp}
\acrshort{api}s and by leveraging \acrshort{lsp} diagnostics and code actions to
provide suggested spellings.
...
JavaScript, TypeScript, CSS, and HTML support is built into VS Code, so it is
exempt from these comparisons.

% \begin{table}
% \caption{Popularity of programming languages among VS Code users and StackOverflow survey respondents.}
% \begin{tabular}{|l|l|}
% 	foo & bar \tabularnewline \hline
% 	1 & 2
% \end{tabular}
% \end{table}
